    ...
    >
      <Box style={{ ...badge, transition: 'transform .3s, background .3s' }} hoverStyle={{ transform: 'translateY(-3px)', background: '#221d1c' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <span style={{ fontSize: 11, opacity: 0.55 }}>On orders of 6+ bottles</span>
        </div>
      </Box>
      <div style={badge}>
        <Icon size={22} stroke="#c22b45" strokeWidth={1.6}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <span style={{ fontSize: 11, opacity: 0.55 }}>Unopened, within 30 days</span>
        </div>
      </div>
      <div style={badge}>
        <Icon size={22} stroke="#c22b45" strokeWidth={1.6}>
...
export function ShopPage() {
  const { set } = useStore();
  const { featured, hasProducts, emptyTitle, addToCart, buyNow } = useShopView();
  ...
  if (!hasProducts || !featured) {
    return (
      ...
      >
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 500, fontSize: 40, margin: 0, lineHeight: 1.05 }}>
          {emptyTitle}
        </h1>
        <p style={{ margin: 0, opacity: 0.7, maxWidth: '48ch', textWrap: 'pretty' }}>
          We are still sourcing bottles for this shelf. Check back shortly, or browse the rest of the cellar.
        </p>
        <Btn
          onClick={() => set({ cat: 'All', query: '', wishOnly: false, featuredId: null })}
          style={{
            marginTop: 8,
            padding: '12px 22px',
            borderRadius: 12,
    ...
    <>
      ...
      <section style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        <Btn
          ...
          hoverStyle={{ transform: 'translateY(-2px) scale(1.01)', boxShadow: '0 16px 40px rgba(194,43,69,.45)' }}
        >
          <span
            style={{
              position: 'absolute',
